...
export class BuyOrdersService {
  ...
  buyOrderEndpoint: ApiEndPoints;
  constructor(private http: HttpClient) {
    this.buyOrderEndpoint = new ApiEndPoints();
  }
  ...
  getBuyOrders(): Observable<BuyOrderModel []> {
    return this.http.get<BuyOrderModel[]>(this.buyOrderEndpoint.BUY_ORDERS).pipe(
      catchError(err => {
        this.handleError([]);
